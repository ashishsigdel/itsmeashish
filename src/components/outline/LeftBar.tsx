      ...
      <div className="hidden md:flex flex-col gap-4 justify-center items-center w-[40px] h-full border-r border-primary-border-color bg-black/30">
        {links.map(({ href, icon, label }) => (
          <Link key={href} href={href}>
            ...
            >
              {icon}
              <span className="absolute opacity-0 group-hover:opacity-100 group-hover:right-8 group-hover:top-0 bg-white text-black text-xs rounded py-1 px-2 transition-all duration-300 whitespace-nowrap z-[999]">
                {label}
              </span>
            </div>
          </Link>
